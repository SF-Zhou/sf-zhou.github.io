...
2. 然后我又用 AI 帮我写了几个 crate 来实现后续的功能，包括：
   1. [blkpath](https://crates.io/crates/blkpath)，用来查询某个文件所在的 block device 路径；
   2. [blkmap](https://crates.io/crates/blkmap)，用来查询某个文件某个 range 对应的 extent 物理地址信息；
   3. [blkreader](https://crates.io/crates/blkreader)，综合上述两个 crate，直接在 block device 上读取某个文件某个 range 的数据，包括 unwritten 状态的 extent。
