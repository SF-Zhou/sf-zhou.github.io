...
`EnvWrapper` is a simple wrapper to `Env`. As mentioned in comments, it may be useful to clients who wish to override just part of the functionality of another `Env`. In `db/env.cc`:
...
In `Add` function, `key` and `value` are encoded into `buf`, then it is insert into skiplit `table_`.
