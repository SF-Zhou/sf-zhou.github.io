...
这里申请不同次数的、不同大小的 int 数组，分别使用内存池和 C++ 原生 的 new 和 delete 方法，测量执行的时间并打印。measure_time 函数可以计算 lambda 函数多次执行的平均平均时间，该函数于 functools.hpp 中定义，详细代码可以参看 GitHub。最后使用 clang++ 编译并开启 O2 优化，得到的结果如下：
