...
Paxos 协议执行过程中包含大量的消息收发，网络通信的性能就十分重要。PaxosStore 中使用了 Epoll 和边缘触发获得高性能，并实现了一定程度的封装提高易用性。下一篇将继续分析 PaxosStore 核心的消息收发组件 `IOWorker`。
