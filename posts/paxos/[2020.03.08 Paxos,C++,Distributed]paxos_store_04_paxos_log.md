...
[本系列](https://sf-zhou.github.io/#/Paxos)的[上一篇](/paxos/paxos_store_03_consensus.html)里分析了 PaxosStore 中共识协议的实现，本篇将聚焦 Paxos 协议中 PaxosLog 的实现。博文标题为了对齐一下，强行翻译为协议日志，不必太在意 :D
...
struct EntityInfo_t {
  ...
  volatile uint64_t iMaxPLogEntry;        // 本机正在处理的最大 Entry 编号，默认值 INVALID_ENTRY
  volatile uint64_t iMaxChosenEntry;      // 本机知道的最大的已经确定的 Entry 编号
  volatile uint64_t iMaxContChosenEntry;  // 本机获取到的最大的已经确定的 Entry 编号
