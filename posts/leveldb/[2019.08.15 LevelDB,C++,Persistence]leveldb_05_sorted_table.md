...
在介绍 `TableBuilder` 之前，需要先介绍它的依赖：`Block` 和 `BlockBuilder`。先看  [`table/block_builder.h`](https://github.com/google/leveldb/blob/master/table/block_builder.h)：
...
接下来是 `TableBuilder::Add` 等函数的实现。由于依赖的原因，这里需要先看 [`table/format.h`](https://github.com/google/leveldb/blob/master/table/format.h)：
