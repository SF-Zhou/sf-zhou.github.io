...
Unix 下一切皆文件（Everything is a file），对 Socket 文件描述符 `fd` 的操作与一般文件操作无异，常用系统调用 `read` / `write` / `close` / `fcntl` / `fsync` 均正常使用。Socket 相关的函数声明于 `sys/socket.h` 中，有详细的注释，这里摘录部分：
